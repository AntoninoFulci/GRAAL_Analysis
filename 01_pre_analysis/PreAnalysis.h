...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   PreAnalysis(std::string files = "", const std::vector<std::string>& branchNames = {});
   virtual ~PreAnalysis();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TChain *tree);
   virtual void     Loop(std::string output_file = "PreAnalysis.root");
   virtual bool     Notify();
   virtual void     Show(Long64_t entry = -1);
   virtual void     EnableOnly(TChain *tree, const std::vector<std::string>& branchNames);
};
...
void PreAnalysis::EnableOnly(TChain *tree, const std::vector<std::string>& branchNames) {
    // Disable all branches
    tree->SetBranchStatus("*", 0);
    // Enable only the specified branches
...
Int_t PreAnalysis::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t PreAnalysis::LoadTree(Long64_t entry)
{
...
void PreAnalysis::Init(TChain *tree)
{
...
void PreAnalysis::Show(Long64_t entry)
{
...
Int_t PreAnalysis::Cut(Long64_t entry)
{
