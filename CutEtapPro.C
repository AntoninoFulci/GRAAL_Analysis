// funzione che assume in input in odine: il vettore dei fotoni, il vetotre dei neutroni in avanti, il vettore dei deutoni in avanti, e ritorna 1 se l'evento rispetta le condizioni imposte e sia contenuto all'interno della regione simulata Eg vs theta Pro
bool CutEtapPro(vector<TLorentzVector> cphoton, vector<TLorentzVector> fneutron, vector<TLorentzVector> fDeuteron, vector<TLorentzVector> proton, double Eg){
  bool val=0;

  TCutG *cutg = new TCutG("cutg",22);
  cutg->SetVarX("Energy gamma(GeV)");
  cutg->SetVarY("proton theta(degree)");
  cutg->SetTitle("Graph");
  ...
  if(cphoton.size()>=2 && fneutron.size()==0 && proton.size()==1 && fDeuteron.size()==0){
    TLorentzVector *protone = proton.data();
    if(cutg->IsInside(Eg,protone[0].Theta()*TMath::RadToDeg())){
     val=1;
    }
  }
